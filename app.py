...
from manage_data import get_json_content, load_feeds, parse_and_append
            ...
            ui.sidebar(
                ...
                ui.accordion(
                    ui.accordion_panel("Keywords",
                                    ui.input_switch("switch_keys", "Deselect all", False),
...
def server(input, output, session):
    ...
    try:
        saved_rows = get_json_content("kept_rows.json")
    ...
    def _():
        ...
        if value:
            keywords = get_json_content("keywords.json")
            ui.update_checkbox_group(
                "checkbox_keys",
    ...
    def df_all_feeds():
        ...
        feeds_selected = get_json_content("feeds_selected.json")
        ...
        if len(set_patterns) < 1 and input.switch_keys() is True:
            ui.notification_show("0 keyword selected", 
                                type='warning',
                                duration=5,
                                id="id_no_key")
            df = filtered_df
        elif len(set_patterns) < 1 and input.switch_keys() is False:
            df = filtered_df
        else: 
        ...
        date_limit = pd.to_datetime(date_limit, dayfirst = True)
...
# KEYWORDS MANAGEMENT
    ...
    def _():
        ...
        keys = get_json_content("keywords.json")
        ...
        if input.new_keyword() != "":
            ...
            try:
                keywords_selected = get_json_content("keywords_selected.json")
                print("json loaded")
...
# FEEDS MANAGEMENT
    ...
    async def _():
        await waiting_notif(adding_feed(), "Adding feed", "id_add_feed")

    async def adding_feed():
        feeds = get_json_content("feeds_dict.json")
        ...
        if input.new_name() != "":
            ...
            try:
                ...
                ui.update_checkbox_group(  
                    id = "checkbox_feeds",  
                    choices = sorted(list(feeds["feed_title"])),
                    selected=list(feeds["feed_title"])
                )
    ...
    async def _():
        await waiting_notif(delete_feed(), "Deleting feed", "id_del_feed")
